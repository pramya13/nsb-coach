...
const DOWNLOAD_DIR = path.join(process.cwd(), 'scripts', 'pdfs-ms');
const OUTPUT_FILE = path.join(process.cwd(), 'scripts', 'questions-ms.json');
...
async function extractPdfLinks() {
  ...
  const html = (await fetchUrl(BASE + '/wdts/nsb/Regional-Competitions/Resources/MS-Sample-Questions')).toString();
  const matches = [...html.matchAll(/href="(\/-\/media\/wdts\/nsb\/pdf\/MS-Sample-Questions\/[^"]+\.pdf)"/gi)];
...
async function downloadPdf(relUrl) {
  const filename = relUrl.replace(/\/-\/media\/wdts\/nsb\/pdf\/MS-Sample-Questions\//i, '').replace(/\//g, '_');
